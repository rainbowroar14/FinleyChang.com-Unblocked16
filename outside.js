...
(function (global) {
  ...
  function isCrosswalk(x, y) {
    return y >= 19 && y <= 22 && x === CROSSWALK_X;
  }
  ...
  function initNPCs() {
    ...
    walkers = [];
    const paths = [
      { x: 4 * TILE, y: 16 * TILE, dir: 1, minX: 2 * TILE, maxX: 42 * TILE },
      { x: 24 * TILE, y: 17 * TILE, dir: -1, minX: 14 * TILE, maxX: 40 * TILE },
      { x: 12 * TILE, y: 18 * TILE, dir: 1, minX: 6 * TILE, maxX: 30 * TILE },
    ];
    paths.forEach(function (p, i) {
      walkers.push({
        ...
        frame: 0,
        timer: 0,
        shirt: ["#4a6a9a", "#8a5a4a", "#5a7a5a"][i % 3],
        busy: false,
        fleeing: false,
        removed: false,
        bumpCd: 0,
  ...
  function update(dt) {
    troops.forEach(function (s) {
      ...
      s.x += s.speed * dt;
      s.step += dt;
      if (s.x > W * TILE + 80) s.x = -100 - Math.random() * 120;
      if (s.x < -120) s.x = W * TILE + 40;
    });
  ...
  function drawTroop(cam, s) {
    const x = s.x - cam.x;
    const y = s.y - cam.y;
    const leg = s.stopped ? 0 : Math.floor(s.step * 8) % 2;
    drawPixel(x + 6, y + 28 + leg, 6, 10, "#1a1a22");
    drawPixel(x + 16, y + 28 - leg, 6, 10, "#1a1a22");
    drawPixel(x + 4, y + 14, 20, 16, "#3d4a32");
    drawPixel(x + 8, y + 4, 12, 12, "#c8a888");
    drawPixel(x + 22, y + 4, 8, 20, "#2a3020");
  ...
  function drawWalker(cam, w) {
    ...
    drawPixel(x + 8, y + 36 + leg, 6, 10, "#2a2a32");
    drawPixel(x + 18, y + 36 - leg, 6, 10, "#2a2a32");
    drawPixel(x + 6, y + 18, 20, 20, w.shirt);
    drawPixel(x + 10, y + 6, 12, 14, "#e8c4a0");
    drawPixel(x + 8, y + 4, 16, 6, "#3a2820");
  }
